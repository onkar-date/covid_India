...
export class CovinService {
  ...
  async getVaccinationSessionByPin(pincode, date): Promise<any> {
    ...
    const params = new HttpParams()
      .set('pincode', pincode)
      .set('date', date);
    return await this.wrapperService.Api(ApiMethod.GET, sessionByPinUrl, true, params);
  }

  // API to get planned vaccination sessions on a specific date in a given district.
  async getVaccinationSessionByDistrict(districtId, date): Promise<any> {
    const districtsUrl = urlHelper.createUrl([
      environment.variables.baseUrlCovin,
      CovinPrimary.appointmentSession,
    ...
    const params = new HttpParams()
      .set('pincode', districtId)
      .set('date', date);
    return await this.wrapperService.Api(ApiMethod.GET, districtsUrl, true, params);
  }
}
